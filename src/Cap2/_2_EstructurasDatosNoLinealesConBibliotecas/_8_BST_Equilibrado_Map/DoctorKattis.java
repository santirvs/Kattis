...
//             No hay grandes diferencias de planteamiento, sospecho del rendimiento de la implementación de la librería de Java
public class DoctorKattis {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String,Gato> mapaGatos = new HashMap<>();
        int llegada = 0;
        for (int i = 0; i < numEventos; i++) {
            String[] evento = in.readLine().split(" ");
            switch (evento[0]) {
                case "0": {  //arrival
                        Gato g = new Gato(evento[1], Integer.parseInt(evento[2]), llegada);
                        colaGatos.add(g);
                        mapaGatos.put(evento[1], g);
                        llegada++;
                    }
                    break;
                case "1": { // Update infection level
                        Gato gato = mapaGatos.get(evento[1]);
                        //Sacar al gato de la cola.
                        //Hay que sacarlo antes de actualizar el nivel para que el comparador lo encuentre
                        colaGatos.remove(gato);
                        gato.nivel += Integer.parseInt(evento[2]);
                        //Volver a meter el gato una vez incrementado el nivel
                        colaGatos.add(gato);
                    }
                    break;
                case "2": { // Treated
                        Gato g = mapaGatos.remove(evento[1]);
                        colaGatos.remove(g);
                    }
                    break;
                case "3": {// Query
