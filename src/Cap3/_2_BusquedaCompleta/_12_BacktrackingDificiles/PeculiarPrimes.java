...
// v4. En lugar de multiplicar cada resultado por todos los números primos, voy a
//     multiplicar cada número primo por sí mismo y por el siguiente
//     --> AC.

// Explicación de la mejor poda de la v4:
//     En el backtracking anterior estamos teniendo mucho solapamiento de resultados.
//     Por ejemplo, si tenemos los primos 2, 3, 5
//     Se está probando el 2*3*5, pero también el 2*5*3, y el 3*2*5, etc...
//     No se está introduciendo ningún orden en el backtracking.
//     En la v4, al multiplicar por el mismo primo y luego por el siguiente, se evita este solapamiento.
...
public class PeculiarPrimes {
    ...
    private static void backtracking(long multiplo, int indicePrimo) {
        // Caso directo: Si el multiplo está fuera del rango, abortar
        if (multiplo > rangoSuperior) return;
        ...
        // Si hemos recorrido todos los primos, no hay más que hacer
        if (indicePrimo >= primos.length) return;

        // Recorrer la lista de números primos y hacer backtracking con su producto
        backtracking(multiplo * primos[indicePrimo], indicePrimo); // Multiplicar por el mismo primo
        backtracking(multiplo, indicePrimo + 1); // Pasar al siguiente primo sin multiplicar
