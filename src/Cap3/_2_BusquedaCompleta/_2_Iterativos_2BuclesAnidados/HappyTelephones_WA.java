...
//        Dejo esta versión aquí (que me ha quedado muy bonita!) y lo hago en otro fichero

import java.io.DataInputStream;
import java.io.FileInputStream;
import java.io.IOException;

public class HappyTelephones_WA {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (numLlamadas != 0 || numIntervalos != 0) {
            ...
            // Procesar cada intervalo y calcular la cantidad máxima de las llamadas en ese intervalo
            for (int i = 0; i < numIntervalos; i++) {
                int inicioIntervalo = scan.nextInt();
                int finIntervalo = inicioIntervalo + scan.nextInt();
                int maxLlamadas = 0;

                // Calcular la cantidad máxima de llamadas en el intervalo
                int[] llamadasEnIntervalo = new int[maxFin-minInicio+1]; // Array para almacenar las llamadas en el intervalo
                ...
                for (int llamada = 0; llamada < numLlamadas; llamada++) {
                    ...
                    // Incrementar el contador de llamadas en el rango solapado
                    for (int j = Math.max(inicioIntervalo, inicioLlamada); j < Math.min(finIntervalo, finLlamada); j++) {
                        llamadasEnIntervalo[j-minInicio]++;
                        maxLlamadas = Math.max(maxLlamadas, llamadasEnIntervalo[j-minInicio]);
                    }
                }

                //Mostrar el resultado
                System.out.println(maxLlamadas);
            }
